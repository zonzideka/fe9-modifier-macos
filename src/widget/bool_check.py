...
class BoolCheck(Customize, QCheckBox):
    ...
    @staticmethod
    def _state_int(state):
        # PySide6 6.x returns Qt.CheckState enum from checkState(); old code expected int.
    ...
    def refresh(self):
        ...
        if not self._text:
            ...
            self.setText(self._text[self._state_int(self.checkState())][0])
    ...
    def rewrite(self):
        if not self._text:
            ...
            offset = self.offset
            state_int = self._state_int(self.checkState())
            text, value = self._text[state_int]
            if state_int == self._state_int(Qt.PartiallyChecked):
